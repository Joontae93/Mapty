...
var _leaflet = require("./js/Leaflet");
...
new class App {
    ...
    selectedBusiness = {};
    constructor(){
        (0, _leaflet.displayMap)();
        (0, _sidebar.handleSidebar)();
        const toggle = document.querySelector(".switch input");
        toggle.addEventListener("change", ()=>this.reduceMotion = toggle.checked);
        this.init();
    }
    #getThePlaces = async function() {
        ...
        const params = `?_fields=title,content,id,acf&order=asc&orderby=title&per_page=10&page=${this.#pageQuery}`;
        try {
            const response = await (0, _events.getData)(businessURL, params);
    ...
    #showLoadingMessage(initialLoad = true) {
        const loadingDiv = document.getElementById("loading");
        const loadingMessage = initialLoad ? "Loading Chahtapreneurs..." : "Loading More Chahtaprenuers...";
        loadingDiv.innerText = loadingMessage;
        if (this.isLoading) !loadingDiv.classList.contains("show") && loadingDiv.classList.add("show");
        else loadingDiv.classList.remove("show");
    }
    #getTheCoordinates = async function() {
        const promises = [];
        this.places.forEach((place)=>{
            if (place.acf.remote === true) return;
            const params = `?access_key=${"632a67a62cec5785b299ba2207fea0ac"}&query=${place.acf.address}&json=true`;
            const promise = (0, _events.getData)("http://api.positionstack.com/v1/forward", params).then(({ data  })=>{
                const coords = [
                    data[0].latitude,
                    data[0].longitude
                ];
    ...
    #getTheMarkup() {
        ...
        for (const place of this.places){
            ...
            const content = place.content.rendered.slice(0, 141) + "...";
            const { acf , id  } = place;
            markup += `
      <li class="business ${acf.remote === false ? "business--physical" : "business--remote"}" data-id=${id}>
        <h3 class="business__title">${title}</h3>
        ${acf.address && `<span class="business__meta">${acf.address}</span>`}
        <div class="business__details">
        ${content}</div>
        ${acf.remote === false ? "" : `<p class="business--online">Online Only</p>`}
      </li>`;
        }
        return markup;
    }
    #generateMarker() {
        L.marker(this.selectedBusiness.coords).addTo((0, _leaflet.map)).bindPopup(L.popup({
            maxHeight: 250,
            maxWidth: 250,
            className: "popup",
            content: `<h3>${this.selectedBusiness.title.rendered}</h3><span class="business__meta">${this.selectedBusiness.acf.address}</span><p>${this.selectedBusiness.content.rendered}</p>`
        })).openPopup();
    }
    #setMapView({ target  }) {
        ...
        if (business) {
            ...
            const [setView] = this.places.filter((place)=>place.id == id);
            if (setView === undefined) {
                alert("Hmm...something went wrong. Try selecting a different business.");
                return;
            }
            ...
            if (isUndefined) {
                alert("Hmm...something went wrong. Try selecting a different business.");
                return;
            } else if (this.reduceMotion) (0, _leaflet.map).setView(setView.coords, 13);
            else (0, _leaflet.map).flyTo(setView.coords, 13, {
                duration: 1.5
            });
        }
    }
    #getMoreBusinesses(ev) {
        if (this.#container.scrollTop + this.#container.clientHeight >= this.#container.scrollHeight) {
            ...
            this.#container.removeEventListener("scroll", this.#getMoreBusinesses);
            this.init();
        }
    }
    init() {
        ...
        this.#getThePlaces().then(()=>{
            this.#getTheCoordinates().then(()=>{
                this.#container.insertAdjacentHTML("beforeend", this.#getTheMarkup());
                ...
                this.#showLoadingMessage();
                this.#container.addEventListener("scroll", this.#getMoreBusinesses.bind(this));
            }).catch((err)=>console.error(err));
        }).catch((err)=>console.error(err));
    }
}();

},{"./js/Events":"4Y8yc","./js/Leaflet":"6hQt3","./js/Sidebar":"cOcpq"}],"4Y8yc":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "getData", ()=>getData);
parcelHelpers.export(exports, "getInfo", ()=>getInfo);
const ROOT_URL = "https://cnodev.wpengine.com/wp-json/wp/v2";
const locationsURL = `${ROOT_URL}/locations?_fields=title,acf,id`;
const eventsURL = `${ROOT_URL}/events?_fields=title,id,acf,slug`;
...
async function getInfo() {
    try {
        ...
        let ids = [];
        events.forEach((el)=>ids.push(el.id));
        const locationsRaw = await getData(locationsURL, `&include=${ids}`);
        const data = {
            events: events,
...
},{}],"6hQt3":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"leaflet/dist/leaflet":"k7zno","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"k7zno":[function(require,module,exports) {
/* @preserve
...
},{}],"cOcpq":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "handleSidebar", ()=>handleSidebar);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["47Zjz","8ZNvh"], "8ZNvh", "parcelRequire9d23")
